...
function getTsConfigWithStrings(tsConfigPath: string): TsConfigOrError {
  const parseResult = getParsedCommandLineOfConfigFile(
    tsConfigPath,
    undefined,
    ts.sys as any,
  );
...
function compilerOptionsWithEnumValues(
  output: { config?: any; error?: ts.Diagnostic },
  config: CliArgs,
) {
  const configFile = output.config;
  const dir = path.dirname(config.tsConfigPath);
  const tsConfigWithEnumValues = parseJsonConfigFileContent(
    configFile,
    sys,
    dir,
  );
...
export function inspect(config: CliArgs):
  ...
  const compilerOptionsInTsConfigWithEnums = compilerOptionsWithEnumValues(
    output,
    config,
  );
  const parseResult = getParsedCommandLineOfConfigFile(
  ...
  const tsConfigRootPropertiesWithStrings = getTsConfigWithStrings(
    config.tsConfigPath,
  );
  ...
  for (const option in computedOptions) {
    if (!providedKeys.has(option)) {
      ...
      const implied = computedOptions[optionKey].computeValue(
        compilerOptionsInTsConfigWithEnums,
      );
  ...
  const implied = config.withDefaults
    ? withAndWithoutDefaults.withDefaults
    : withAndWithoutDefaults.withoutDefaults;
  let compilerOptions: CompilerOptions = {
    ...tsConfigRootPropertiesWithStrings.compilerOptions,
    ...implied,
  };
  ...
  const hideFiles = config.showFiles
    ? {}
    : {
        files: output.config.files,
        exclude: output.config.exclude,
        include: output.config.include,
      };
  ...
  const sortedCompilerOptions = sortObjectByKeysArray(
    resultingTsConfig.compilerOptions,
    order,
  );
  return {
    onlyGenerated: withAndWithoutDefaults.withoutDefaults,
...
function sortObjectByKeysArray(
  obj: CompilerOptions,
  keys: (keyof CompilerOptions)[],
) {
...
export function addDefaultCompilerOptions(
  compilerOptions: CompilerOptions,
): CompilerOptions {
...
const enumMapping = <T extends CompilerOptionsValue>(
